...
for nums in mkt_list:
    if nums == 1:
        ...
        if tickers != 'none':
            ...
            pdf_filename = f'{pdf_folder}/{current_date}_{idx_code}.pdf'
            with (PdfPages(pdf_filename) as pdf):
                ...
                short_term_movers = all_dfs_df[['adv_dec_ratio',
                                                '>4%D', '>6%2D', '>7%3D', '>8%4D', '>9%5D', '>10%6D', '>11%7D',
                                                '>12%8D',
                                                '<4%D', '<6%2D', '<7%3D', '<8%4D', '<9%5D', '<10%6D', '<11%7D',
                                                '<12%8D',
                                                ]].copy()
...
# pdf_filename = f'{pdf_folder}/{current_date}_{idx_code}.pdf'
pdf_to_open = os.path.join(pdf_folder, f'{current_date}_^BVSP.pdf')
if os.path.exists(pdf_to_open):
    try:
        os.startfile(pdf_to_open)  # Open PDF using the default viewer on Windows
    except AttributeError:
        os.system('open "{}"'.format(pdf_to_open))  # Open PDF using the default viewer on macOS or Linux
else:
